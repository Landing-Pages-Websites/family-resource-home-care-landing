...
- **Composition judgment:** evaluate the whole section and its neighbors, not only the named
  element. State the section job and ranked priorities; remove dead space; make every element
  earn its footprint; inspect approved assets/data for native reusable parts; compare section
  height and full-page rhythm before/after; restructure or prune when that fixes the root issue.
- **Interior marketing pages:** classify the page before composing it. Inherit the parent site's
  tokens/type/nav/interaction language, NOT its signature homepage layouts. State the page job,
  visitor decision sequence, page-specific signature, and section-purpose map in the prompt-file.
  When no exact design exists, use the `frontend-design` interior-page image-first pass before code.
  Concepts guide composition; rebuild them as semantic responsive UI.
...
child pages that copy the homepage template, repeated card walls, or conceptual visuals that
overcomplicate simple feature content.
